document.addEventListener("DOMContentLoaded", async function () {
    ...
    // Subtract thirty hours from current date and time
    const currentDateTimeMinus00Hours = subtractHoursFromDate(currentDateTime, 30);
                ...
                .then(() => {
                    ...
                    for (const locData of combinedData[0][`assigned-locations`]) {
                        ...
                        if (assignedTimeSeries && assignedTimeSeries.length == 2) {
                            ...
                            netmissApiUrl = setBaseUrl + `timeseries?name=${netmissTsid}&begin=${currentDateTimeMinus00Hours.toISOString()}&end=${currentDateTimePlus190Hours.toISOString()}&office=${office}`;
                            // netmissApiUrl = setBaseUrl + `timeseries?name=${netmissTsid}&office=${office}&begin=${currentDateTimeMinus00Hours.toISOString()}&end=${currentDateTimePlus190Hours.toISOString()}`;
                        } else {
                            console.error(
